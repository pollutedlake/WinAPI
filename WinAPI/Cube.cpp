...
void Cube::update(void)
{
	...
	float _ry = 20 * cos(PI + _camera.tilt);
	float _rz = 20 * sin(PI + _camera.tilt);
	float _rx = _ry * cos(PI * 3 / 2 + _camera.pan);
	_ry = _ry * sin(PI * 3 / 2 + _camera.pan);
	_camera.position._x = _rx;
	_camera.position._y = _ry;
	_camera.position._z = _rz;
...
void Cube::render(void)
{
	...
	DeleteObject(myPen);
	char s[256];
	wsprintf(s, "%f, %f, %f", _camera.position._x, _camera.position._y, _camera.position._z);
	TextOut(getMemDC(), 0, 0, s, strlen(s));
}
