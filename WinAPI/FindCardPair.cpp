...
HRESULT FindCardPair::init(void)
{
	GameNode::init();
	HBRUSH hBrush = NULL;
	for (int i = 0; i < 20; i++)
...
void FindCardPair::release(void)
{
	GameNode::release();
	for (int i = 0; i < 20; i++)
	{
...
void FindCardPair::render(HDC hdc)
{
	for (int i = 0; i < 20; i++)
	{
		if (cards[i]->getOpen())
...
LRESULT FindCardPair::MainProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	HDC hdc;
	PAINTSTRUCT ps;
	switch (iMessage)
